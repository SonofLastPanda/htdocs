...
<html>
  ...
  <div class="container">
      ...
      <input type="text" placeholder="<?php echo decrypt($row[0]);?>" name="new_useremail" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" value="<?php echo decrypt($row[0]);?>"><br><br>
      <label><b>Password:</b></label>
      <input type="password" placeholder="Enter New Password" name="new_userpassword" id="new_userpassword" minlength= "8" value="<?php echo decrypt($row[1]);?>"><br><br>
      ...
      <label><b>Remove favorited destination:</b></label> <select name="TO"> <!--Drop down list -->
        ...
        <?php 
          ...
          $result2 = mysqli_query($link,"SELECT DISTINCT country.country_name FROM country JOIN bookmark ON country.country_id = bookmark.country_id JOIN users ON users.user_id = bookmark.user_id WHERE users.username = '$old_username'"); 
